...
app.post("/api/addTuna", async function(req, res) {
    try {
        const contract = await fabricNetwork.connectNetwork(
            "connection-producer1.json",
            "wallet/wallet-producer1"
        );
        let tuna = {
            id: req.body.id,
...
app.post("/api/setPosition", async function(req, res) {
    try {
        const contract = await fabricNetwork.connectNetwork(
            "connection-producer2.json",
            "wallet/wallet-producer2"
        );
        let tx = await contract.submitTransaction(
            "setPosition",
...
app.get("/api/getHistorySushi/:id", async function(req, res) {
    try {
        ...
        const historyTuna = JSON.parse(
            (
                await contract.evaluateTransaction(
                    "getHistory",
                    actualSushi.tunaId.toString()
                )
            ).toString()
        );
        const actualTuna = JSON.parse(
            (
                await contract.evaluateTransaction(
                    "queryTuna",
                    actualSushi.tunaId.toString()
